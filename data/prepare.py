...
def copy_audio_to_local():
    """
    Copy audio from Google Drive to the local Colab SSD.
    Local disk is much faster than Drive for repeated reads.
    Skips the copy if the folder already exists, so it is safe to re-run.
    """
...
def remap_path(windows_path: str) -> str:
    """
    Remap Windows-style paths from the manifest CSVs to local SSD paths.
    The manifest was made on Windows, so paths look like
    ...
    return p  # already a local path or not recognised
...
# Step 3: preprocess audio and text into model-ready arrays.
...
def make_preprocess_fn(processor: Wav2Vec2Processor):
    ...
    For audio it produces input_values (normalised float array)
    and attention_mask (1 for real audio, 0 for padding).

    For text it tokenises Dutch text into integer IDs.
    For example 'hallo' becomes [8, 1, 12, 12, 15].

    Note: do NOT call set_format('torch') after this. NumPy 2.0 has a
    bug with variable-length arrays in that mode. The collator does the
    numpy to torch conversion instead.
    """
...
def build_datasets(df_train: pd.DataFrame, df_val: pd.DataFrame,
    ...
    """
    Convert the dataframes into preprocessed HuggingFace Datasets.
    Results are cached to disk, so re-running is instant.
    The first run takes about 45 min for 18k clips.
    """
