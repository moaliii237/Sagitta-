...
class DataCollatorCTCWithPadding:
    ...
    It does two things per batch:

    1. Dynamic padding.
       Pads each batch to the longest example in that batch,
       not globally. Saves memory compared to padding to the
       longest clip in the whole dataset.

    2. Label masking with -100.
       Replaces pad_token_id (33) in the labels with -100.
       The PyTorch CTC loss skips positions where label == -100.
       Without this the model learns from padding tokens, which
       gives negative loss and corrupts training.
       This fix resolved the WER above 100% bug in V9.

    I call feature_extractor.pad() and tokenizer.pad() directly
    (not processor.pad()) to avoid the deprecated
    as_target_processor warning and to make sure attention_mask
    is always returned.
    """
