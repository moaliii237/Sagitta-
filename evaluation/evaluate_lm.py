# evaluation/evaluate_lm.py: build the LM decoder and run the LM eval.
...
def build_decoder(processor):
    """
    Build the CTC beam search decoder with the call center LM.

    Alpha (LM weight): how much to trust the LM versus the acoustic model.
      Best found via grid search: 0.3 (low, because the acoustic model is strong).
    ...
    Unigrams are the domain vocabulary (1081 call center words).
    This is the hotwords mechanism, the decoder prefers these words.

    The vocab must be ordered by token ID, with | replaced by a space
    and [PAD] replaced by an empty string.
    """
    # Load the domain hot words.
    ...
    # Build the vocab ordered by token ID.
...
def run_full_lm_eval(model, val_dataset, data_collator, processor, decoder):
    ...
    for batch in tqdm(loader, desc='Greedy + LM eval'):
        ...
        # LM plus unigrams, one example at a time.
    ...
    print(f'  {"Delta":14} {l_wer-g_wer:>+9.1f}% {l_cer-g_cer:>+9.1f}%')
    print('='*58)
