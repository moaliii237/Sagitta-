...
# Transcribes real 8kHz Sagitta call center audio and compares
# against WhisperX reference transcripts.
...
def transcribe_audio(audio_path: str, model, processor, decoder,
    ...
    MAX_CHUNK_SEC = 30s is safe for wav2vec2 on A100.
    Chunks are joined and cleaned after decoding.
    Memory is freed between chunks to avoid OOM.
    """
...
def main():
    ...
    for pair in tqdm(pairs, desc='Transcribing Sagitta calls'):
        ...
        try:
            ...
            print(f'  ⚠️  Skipped {pair["id"][:40]}: {e}')
            torch.cuda.empty_cache()
    ...
    print('\n' + '='*62)
    print('  V10 vs WhisperX — Real Sagitta 8kHz Calls')
    print('='*62)
    print(f'  Calls evaluated : {len(valid)} / {len(pairs)}')
    print(f'  {"V10 Greedy":<20} avg WER vs WhisperX: {avg_greedy:.1f}%')
    print(f'  {"V10 + Sagitta LM":<20} avg WER vs WhisperX: {avg_lm:.1f}%')
    print('='*62)
    ...
    print('   WER ~52% is expected — model was trained on 16kHz clean audio.')
    print('   Sagitta calls are 8kHz telephone audio (different acoustic domain).')
    print('   Next step: fine-tune on real Sagitta 8kHz calls for V11.')
