...
# In Colab, Drive is mounted at /content/drive. When running locally,
# point this to wherever you synced Drive.
DRIVE_ROOT = '/content/drive/MyDrive/ArcumSagitta'

# Input: the already-trained V9 model.
# V9 was trained on CGN telephone speech plus internal calls.
V9_MODEL_PATH     = f'{DRIVE_ROOT}/v9/models/gronlp'
V9_PROCESSOR_PATH = f'{DRIVE_ROOT}/v9/processor/gronlp'

# Output: where V10 is saved.
...
# Belgian Dutch (14,985 clips) plus CSS10 Dutch (6,431 clips).
...
# Windows to local path remapping.
# The manifest CSVs were created on Windows, so paths look like:
# D:\dutch_asr_prep\output\belgian_audio\clip.wav
# I remap them to local SSD paths after copying from Drive.
WINDOWS_PATH_MAP = {
...
# Low LR (3e-5) because the model already knows Dutch from V9.
# A high LR would cause catastrophic forgetting.
LEARNING_RATE      = 3e-5
NUM_TRAIN_EPOCHS   = 5
BATCH_SIZE_TRAIN   = 16
BATCH_SIZE_EVAL    = 8
...
LM_ALPHA = 0.3   # LM weight, low because the acoustic model is strong
