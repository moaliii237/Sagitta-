# =============================================================
# training/train.py — Fine-tune V9 → V10
...
def get_training_args() -> TrainingArguments:
    """
    TrainingArguments — key decisions:

    learning_rate = 3e-5
        Low to avoid catastrophic forgetting of V9's Dutch knowledge.

    fp16 = True, bf16 = False
        fp16 halves VRAM and speeds up A100.
        bf16 caused NaN loss in V9 so it stays off.

    gradient_checkpointing = False
        Caused tensor mismatch errors in V9. A100 has enough VRAM.

    gradient_accumulation_steps = 2
        Effective batch = 16 × 2 = 32 without loading 32 clips at once.

    group_by_length = True
        Batches similar-duration clips → less padding waste → faster.

    load_best_model_at_end = True
        Loads the checkpoint with lowest eval_loss, not the last one.
    """
...
def main():
    ...
    # The Trainer uses this for padding during eval — for audio
